"""wait 语义重测 driver：两 rank NCCL 子进程（各绑一张 3090）+ 汇总打印。

用法（盒子上）：:

    /root/miniconda3/bin/python run_wait_recheck.py

每个 rank 输出实验 1/2/3 的测量 JSON；本 driver 只是启动两进程并打印各自结果，
对比留给调用方（重测结论见 docs/todo-revisit-wait-semantics.md 的映射表）。
"""
...
_WORKER = os.path.join(_HERE, "wait_recheck_worker.py")
